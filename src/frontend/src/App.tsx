...
const BOOT_LINES = [
  { text: "[  0.001] Kernel initialized", delay: 0 },
  { text: "[  0.042] Loading security modules...", delay: 180 },
  { text: "[  0.118] Mounting encrypted volumes...", delay: 380 },
  { text: "[  0.203] Establishing secure tunnel...", delay: 600 },
  { text: "[  0.287] Authenticating identity...", delay: 820 },
  { text: "[  0.391] Verifying system integrity...", delay: 1050 },
  { text: "[  OK  ] All checks passed.", delay: 1280 },
];
...
function BootSplash({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    for (const { text, delay } of BOOT_LINES) {
      timers.push(setTimeout(() => setLines((prev) => [...prev, text]), delay));
    }

    // Start fade-out after all lines shown + short pause
    timers.push(setTimeout(() => setFadeOut(true), 1700));
    // Unmount after fade completes
    timers.push(setTimeout(() => onDone(), 2200));
  ...
  return (
    ...
    >
      ...
      <div
        style={{ width: "min(560px, 90vw)", position: "relative", zIndex: 1 }}
      >
        {/* Header */}
        <div style={{ marginBottom: "2rem", textAlign: "center" }}>
          <div
            style={{
              fontSize: "0.7rem",
              color: "oklch(0.87 0.28 145 / 0.5)",
              letterSpacing: "0.2em",
              textTransform: "uppercase",
              marginBottom: "0.5rem",
            }}
          >
            SAJIN JOSEPH — SECURE PORTFOLIO v2.0
          </div>
          <div
            style={{
              fontSize: "1.05rem",
              color: "oklch(0.87 0.28 145)",
              textShadow: "0 0 20px oklch(0.87 0.28 145 / 0.7)",
              letterSpacing: "0.05em",
            }}
          >
            Initializing secure portfolio...
          </div>
        </div>
        ...
        <div
          style={{
            ...
            border: "1px solid oklch(0.87 0.28 145 / 0.25)",
            borderRadius: "6px",
            ...
            <div
              ...
              style={{
                color: line.startsWith("[  OK")
                  ? "oklch(0.87 0.28 145)"
                  : "oklch(0.75 0.025 240)",
                textShadow: line.startsWith("[  OK")
                  ? "0 0 8px oklch(0.87 0.28 145 / 0.6)"
                  : "none",
              }}
            >
              {line}
        ...
        >
          <div
            style={{
              height: "100%",
              background: "oklch(0.87 0.28 145)",
              boxShadow: "0 0 10px oklch(0.87 0.28 145 / 0.8)",
              borderRadius: "2px",
              transition: "width 1.3s ease",
              width:
                lines.length >= BOOT_LINES.length
                  ? "100%"
                  : `${(lines.length / BOOT_LINES.length) * 85}%`,
            }}
          />
        </div>
      </div>
